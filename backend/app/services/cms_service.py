...
def scoped(
    ...
    stmt = select(model).where(
        model.organization_id == org_id(db)
    )
...
def scoped_list(db: Session, model):
    return list(
        db.scalars(
            select(model)
            .where(model.organization_id == org_id(db))
...
def get_settings(db: Session):
    settings = db.scalar(
        select(OrganizationSettings).where(
            OrganizationSettings.organization_id == org_id(db)
        )
    )
...
def create_session(db, payload):
    ...
    if data.get("is_current"):
        db.execute(
            ...
            .where(
                AcademicSession.organization_id == org_id(db)
            )
