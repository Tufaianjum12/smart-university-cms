...
def calculate_attendance_percentage(
    ...
    return (
        Decimal(present + late) * Decimal("100") / Decimal(counted)
    ).quantize(
        Decimal("0.01"),
...
def oid(user: User) -> UUID:
    ...
    Tenant identity comes from the authenticated database user rather
    than the ContextVar because attendance service functions already
    receive the authenticated User object.
    """
...
def _ensure_manage_access(
    ...
    if not _teacher_can_manage(db, user, offering):
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to manage attendance for this course offering",
        )
...
def _offering(
    ...
    obj = db.scalar(
        select(CourseOffering).where(
            CourseOffering.organization_id == organization_id,
    ...
    try:
        ...
        if isinstance(exc, IntegrityError):
            raise HTTPException(
                status_code=409,
                detail=(
                    "An attendance session already exists for this "
                    "offering, date and start time"
                ),
            ) from exc
...
def list_sessions(
    ...
    if offering_id is not None:
        ...
        if user.role == UserRole.TEACHER:
            ...
            raise HTTPException(
                status_code=403,
                detail="Students may only view their own attendance",
            )
...
def update_session(
    ...
    merged_date = data.get(
        "session_date",
        obj.session_date,
    )

    merged_start = data.get(
        "start_time",
        obj.start_time,
    )
    ...
    if (
        ...
        raise HTTPException(
            status_code=403,
            detail="Teachers cannot edit completed attendance sessions",
        )
    ...
    try:
        ...
        if isinstance(exc, IntegrityError):
            raise HTTPException(
                status_code=409,
                detail="Attendance session conflicts with an existing session",
            ) from exc
...
def bulk_mark(
    ...
    if obj.status == AttendanceSessionStatus.CANCELLED:
        raise HTTPException(
            status_code=400,
            detail="Cancelled sessions cannot receive attendance",
        )
    ...
    if (
        ...
        raise HTTPException(
            status_code=403,
            detail="Teachers cannot modify completed attendance sessions",
        )
    ...
    if len(enrollment_ids) != len(set(enrollment_ids)):
        raise HTTPException(
            status_code=400,
            detail="Duplicate enrollment IDs in bulk attendance",
        )
    ...
    if len(enrollments) != len(enrollment_ids):
        raise HTTPException(
            status_code=400,
            detail=(
                "Every enrollment must belong to this tenant "
                "and course offering"
            ),
        )
    ...
    if any(
        ...
        raise HTTPException(
            status_code=400,
            detail="Attendance can only be marked for active enrollments",
        )
    ...
    if existing:
        raise HTTPException(
            status_code=409,
            detail=(
                "Attendance already exists for one or more "
    ...
    try:
        ...
        if isinstance(exc, IntegrityError):
            raise HTTPException(
                status_code=409,
                detail=(
                    "Duplicate attendance record or "
...
def student_summary(
    ...
    return {
        ...
        "calculation_rule": (
            "PRESENT and LATE count as attended; ABSENT counts "
            "against attendance; EXCUSED is excluded from the "
            "denominator; CANCELLED sessions are excluded."
        ),
    }
